    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invoice') ? 'is-invalid' : '' }}" type="text" name="invoice" id="invoice" value="{{ old('invoice', $product->invoice) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('manufacturers') ? 'is-invalid' : '' }}" name="manufacturers[]" id="manufacturers" multiple>
                    @foreach($manufacturers as $id => $manufacturer)
                        <option value="{{ $id }}" {{ (in_array($id, old('manufacturers', [])) || $product->manufacturers->contains($id)) ? 'selected' : '' }}>{{ $manufacturer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_purchased') ? 'is-invalid' : '' }}" type="text" name="date_purchased" id="date_purchased" value="{{ old('date_purchased', $product->date_purchased) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes_history') ? 'is-invalid' : '' }}" name="notes_history" id="notes_history">{{ old('notes_history', $product->notes_history) }}</textarea>
